...
class TVAESynthesizer(LossValuesMixin, BaseSingleTableSynthesizer):
    ...
    def encode_to_latent(self, input_data):
        """Wraps the encode_to_latent function of the underlying TVAE model."""
        return self._model.encode_to_latent(input_data)
    
    def decode_from_latent(self, emb):
        """Wraps the decode_from_latent function of the underlying TVAE model."""
        return self._model.decode_from_latent(emb)
    
    
class CTVAESynthesizer(LossValuesMixin, BaseSingleTableSynthesizer):
    """Model wrapping ``CTVAE`` model.
    ...
    def _fit(self, processed_data, condition_indices, feature_indices):
        """Fit the model to the table.
        ...
        self._model = TVAE(**self._model_kwargs)    # bypassing checks?
        self._model.fit(processed_data, discrete_columns=discrete_columns, condition_indices=condition_indices, feature_indices=feature_indices)


    def _sample(self, num_rows, conditions):
        """Sample the indicated number of rows from the model with the help of given conditions.
        ...
        #raise NotImplementedError("CTVAESynthesizer doesn't support conditional sampling.")
