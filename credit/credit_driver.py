...
from discriminatory_age_sex import process_folder
...
def main():
    ...
    t_way_patel = "../Patel_Data/tWay_Concrete_TC/GermanCredit_AI360_Modified_2way_concrete_TC_with_constraint.csv"
    t_way_ours = glob.glob("t_way_samples/credit_train_2_way_covering_array_bin_means_*.csv")

    dataset_patel = "../Patel_Data/Dataset/GermanCredit_AI360_Modified.csv"
    dataset_ours = os.path.join(dataset_dir, "credit_all_data.csv")
    
    t_way_patel_fixed = fix_header_csv_file(t_way_patel)
    dataset_patel_fixed = fix_header_csv_file(dataset_patel)
    
    # Find the discriminatory instances for generated t-way instances
    process_models(t_way_patel_fixed, models_dir, output_dir, results_dir)
    ...
    process_and_save_atn_file_results(results_dir, test_data_file = t_way_patel_fixed, adv_data_file=dataset_patel_fixed, mode='not-equal')
