...
def sample_multivariate_and_save(synthesizer, ht, num_samples, embedding_dimension, train_file_path):
    ...
    df_credit_train = pd.read_csv(train_file_path)
    df_credit_train = df_credit_train.iloc[:, :-1]
    
    num_samples = df_credit_train.shape[0]
    ...
    df_credit_train_transformed_latent_representation = get_latent_representation(df_credit_train, synthesizer, ht)
    
    if isinstance(df_credit_train_transformed_latent_representation, torch.Tensor):
        df_credit_train_transformed_latent_representation = df_credit_train_transformed_latent_representation.detach().cpu().numpy()
    # print(f"\n type of df_credit_train_transformed_latent_representation: {type(df_credit_train_transformed_latent_representation)}, df_credit_train_transformed_latent_representation's shape: {df_credit_train_transformed_latent_representation.shape}")
    
    # Compute the mean and covariance of the training instances
    mean_vector_train = np.mean(df_credit_train_transformed_latent_representation, axis=0)
    print(f"\n type of mean vector: {type(mean_vector_train)}, mean_vector's shape: {mean_vector_train.shape} | mean_vector: {mean_vector_train}")
    
    covariance_matrix_train = np.cov(df_credit_train_transformed_latent_representation, rowvar=False)
...
def sample_normal_and_save(synthesizer, ht, num_samples, embedding_dimension):
    ...
    # Debug: Log details about the synthetic data
    print("Debug: Columns in sampled_synthetic_data before reverse_transform:")
    print(sampled_synthetic_data.columns.tolist())

    print("Debug: First few rows of sampled_synthetic_data:")
    print(sampled_synthetic_data.head())

    # Debug: Log expected columns for reverse_transform
    print("Debug: Expected columns for reverse_transform (HyperTransformer):")
    # print(ht.get_columns())
