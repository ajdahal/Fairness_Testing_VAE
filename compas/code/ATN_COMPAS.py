...
def load_and_equalize_data(test_data_file, adv_data_file, mode):
    ...
    df_xt.columns = [col.replace('-', '-') for col in df_xt.columns]
...
def get_atn(test_data_file, adv_data_file, mode):
    ...
    print(f"\n columns of df_xs: {df_xs.columns}")
    print(f"\n columns of df_xt: {df_xt.columns}")
    continuous_cols = ['age', 'priors_count','juv_fel_count','juv_misd_count','juv_other_count']
    categorical_cols = ['sex', 'age_cat', 'race', 'c_charge_degree']       # c_charge_desc, two_year_recid is removed
    
    # Call function to convert boolean columns (like 'two_year_recid') to categorical
    # df_xs = convert_boolean_to_categorical(df_xs, ['two_year_recid'])
    # df_xt = convert_boolean_to_categorical(df_xt, ['two_year_recid'])
    
    # Calculate KSTest for continuous columns
    ks_scores= []
    for col in continuous_cols:
        # print(f"\n col being processed: {col}")
    ...
    for i in range(len(continuous_cols)):
        for j in range(i + 1, len(continuous_cols)):
            ...
            # print(f"\n col: {col1} and col: {col2} are being processed.")
...
def calculate_and_save_atn_scores(train_file_path, t_way_samples_dir, results_dir, mode='equal'):
    ...
    for test_file_name in os.listdir(t_way_samples_dir):
        ...
        results = sorted(results, key=lambda x: x["Test_File_Name"])
