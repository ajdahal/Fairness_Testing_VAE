...
def sample_multivariate_and_save(synthesizer, ht, num_samples, embedding_dimension, train_file_path):
    ...
    df_COMPAS_train = pd.read_csv(train_file_path)
    df_COMPAS_train = df_COMPAS_train.iloc[:, :-1]
    
    num_samples = df_COMPAS_train.shape[0]
    ...
    df_COMPAS_train_transformed_latent_representation = get_latent_representation(df_COMPAS_train, synthesizer, ht)
    
    if isinstance(df_COMPAS_train_transformed_latent_representation, torch.Tensor):
        df_COMPAS_train_transformed_latent_representation = df_COMPAS_train_transformed_latent_representation.detach().cpu().numpy()
    # print(f"\n type of df_COMPAS_train_transformed_latent_representation: {type(df_COMPAS_train_transformed_latent_representation)}, df_COMPAS_train_transformed_latent_representation's shape: {df_COMPAS_train_transformed_latent_representation.shape}")
    
    # Compute the mean and covariance of the training instances
    mean_vector_train = np.mean(df_COMPAS_train_transformed_latent_representation, axis=0)
    print(f"\n type of mean vector: {type(mean_vector_train)}, mean_vector's shape: {mean_vector_train.shape} | mean_vector: {mean_vector_train}")
    
    covariance_matrix_train = np.cov(df_COMPAS_train_transformed_latent_representation, rowvar=False)
