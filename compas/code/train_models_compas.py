...
processor_export_file_path = os.path.join(models_dir, "compas_preprocessor.pkl")
...
def write_notnn_predictions(model_path):
    ...
    query_file_name = next(iter(glob.glob(os.path.join(t_way_samples_dir, "compas_train_2_way_covering_array_bin_means*.csv"))), None)
    ...
    # Remove the 'two_year_recid' column if it exists
    if 'two_year_recid' in input_data.columns:
        print("Removing 'two_year_recid' column from input data...")
        input_data = input_data.drop(columns=['two_year_recid'])
...
def main():
    ...
    y_train = pd.read_csv(train_file_path, usecols=['two_year_recid']).squeeze()
    y_test = pd.read_csv(test_file_path, usecols=['two_year_recid']).squeeze()
    
    
    # Define numerical and categorical features
    numerical_cols = ['age', 'juv_fel_count', 'juv_misd_count', 'juv_other_count', 'priors_count']
    categorical_cols = list(set(x_train.columns) - set(numerical_cols) - {'two_year_recid'})
    ...
    model_paths = [
        os.path.join(models_dir, "compas_" + model + "_" + str(current_timestamp) + ".pkl")
    ...
    # Train and save NN
    model_path_nn = os.path.join(models_dir, "compas_" + "_nn__" + str(current_timestamp) + ".h5")
