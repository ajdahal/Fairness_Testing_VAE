...
from ATN_COMPAS import calculate_and_save_atn_scores, process_and_save_atn_file_results
from discriminatory_sex_race import process_folder
from t_way_multivariate_discritization_edges_random_means import t_way_multivariate_combinations
from discriminatory_race_sex_combinations_compas import process_models
...
def main():
    ...
    print(f"\nNumber of columns in training data from excluding two_year_recid column: {half_the_columns}")
    ...
    model_path = next((os.path.join(models_dir, f) for f in os.listdir(models_dir) if f.startswith(f"{embedding_dimension}_tvae_COMPAS_synthesizer_model") and f.endswith(".pkl")), None)
    ...
    label_column_name = "two_year_recid"
    ...
    train_models_compas.main(train_file_path, test_file_path, models_dir)
