...
def write_notnn_predictions(model_path):
    ...
    # Remove the 'income' column if it exists
    if 'income' in input_data.columns:
        print("Removing 'income' column from input data...")
        input_data = input_data.drop(columns=['income'])
...
def main(train_file_path, test_file_path, model_dir):
    ...
    y_train = pd.read_csv(train_file_path, usecols=['income']).squeeze()
    y_test = pd.read_csv(test_file_path, usecols=['income']).squeeze()
    
    # Define numerical and categorical features
    numerical_cols = ['age', 'education-num', 'capital-gain', 'capital-loss', 'hours-per-week']
    categorical_cols = list(set(x_train.columns) - set(numerical_cols) - {'income'})
