...
from ATN_adult import calculate_and_save_atn_scores, process_and_save_atn_file_results
...
def main():
    ...
    print(f"\nNumber of columns in training data from excluding income column: {half_the_columns}")
    ...
    model_path = next((os.path.join(models_dir, f) for f in os.listdir(models_dir) if f.startswith(f"{embedding_dimension}_tvae_adult_synthesizer_model") and f.endswith(".pkl")), None)
    ...
    size_of_preprocessed_data = X_train_transformed.shape[1]
    ...
    dataset_patel = "../Patel_Data/Dataset/adult_data_AI360_Modified2.csv"
    dataset_ours = os.path.join(dataset_dir, "adult_all_data.csv")
